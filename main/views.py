...
def guest_default_portal(request):
    try:
        if request.method == "GET":
            ...
            if count_client == 0:
                return render(request, template_name="guest_client_register.html", context=data)
            else:
                Model = UserInformation.objects.get(device_mac_address=client)
                client_status = Model.status
                if client_status == "DEACTIVE":
                    return render(request, template_name="guest_confirm_code.html", context=data)
...
def guest_client_connections(request):
    try:
        if request.method == "GET":
            ...
            return render(request, template_name="guest_client_connections.html", context=data)
    except Exception as error:
        return HttpResponse(error)
